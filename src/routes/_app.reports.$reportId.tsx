...
import { exportReportToSupabaseStorage } from "@/lib/storage/reportExport";
      ...
      <ExportDialog
        ...
        onExport={async (format) => {
          if (format === "PDF") {
            setIsExporting(true);
            try {
              await exportReportToSupabaseStorage(report);
            } finally {
...
function ExportDialog({
  ...
  reportName: string;
  isExporting: boolean;
  onExport: (format: "PDF" | "Workspace") => Promise<void>;
}) {
  async function handleStandard(format: "PDF" | "Workspace") {
    try {
      await onExport(format);
      toast.success(
        format === "PDF"
          ? "PDF downloaded · saved to Drafts"
          : "Saved to workspace · added to Drafts",
      );
      onOpenChange(false);
    } catch (error) {
